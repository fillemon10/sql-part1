 ...
 */
CREATE
OR REPLACE VIEW BasicInformation AS (
    SELECT
        Students.idnr,
        Students.name,
        Students.login,
...
/* FinishedCourses(student, course, grade, credits): for all students,
 ...
 e.g.CHAR(1). */
CREATE
OR REPLACE VIEW FinishedCourses AS (
    SELECT
        Taken.student,
        Taken.course,
        Taken.grade,
...
CREATE
OR REPLACE VIEW PassedCourses AS (
    SELECT
        FinishedCourses.student,
 ...
 */
CREATE
OR REPLACE VIEW Registrations AS (
    SELECT
        WaitingList.student,
        WaitingList.course,
        'waiting' AS status
 ...
 */
CREATE
OR REPLACE VIEW UnreadMandatory AS (
    (
        SELECT
            ...
            AND (
                MandatoryProgram.course NOT IN (
                    ...
                    FROM
                        PassedCourses
                    WHERE
                        Students.idnr = PassedCourses.student
                )
            )
    )
    UNION
            ...
            AND (
                MandatoryBranch.course NOT IN (
                    ...
                    FROM
                        PassedCourses
                    WHERE
                        StudentBranches.student = PassedCourses.student
                )
            )
    )
);
 ...
 */
CREATE
OR REPLACE VIEW PathToGraduation AS (
    SELECT
        Students.idnr as student,
    ...
    FROM
        ...
        LEFT OUTER JOIN (
            ...
            FROM
                ...
                StudentBranches
            WHERE
                PassedCourses.course = RecommendedBranch.course
